    ...
    >
      <div className="flex flex-col top-0 w-full transition-all">
        ...
        <motion.div
          ...
          className="absolute inset-0 bg-gray-50 dark:bg-[#00160E] pointer-events-none"
        />

        <div className="w-full mx-auto px-4 sm:px-6 lg:px-8  flex justify-between items-center h-[80px] relative z-50">
          <Link href={"/"} className="flex">
            <div className="flex flex-col items-start py-4 mt-4 overflow-hidden">
          ...
          <div className="flex items-center clip-customFooter z-50 bg-lime-200 dark:bg-BrandMain-light mt-2 text-BrandMain-dark">
            <button onClick={toggleMenu} className="p-1 sm:p-2">
              {isOpen ? (
                <IoCloseOutline size={42} />
              ) : (
                <HiOutlineMenuAlt2 size={42} />
              )}
            </button>
          </div>
        </div>
              ...
              <div className="nav-details relative inset-0 w-full flex flex-col min-h-screen">
                <div className="w-full flex flex-col gap-2 items-end px-2 sm:px-4 mr-4">
                  {menuItems.map((item) => (
                    <Link key={item.label} href={item.href}>
                      <span
                        onClick={toggleMenu}
                        className={`text-3xl sm:text-5xl md:text-5xl font-semibold font-anton ${
                          pathName === item.href
                ...
                <ul className="w-full md:max-w-[50%] p-2 md:p-4 flex text-normal sm:text-xl md:text-5xl flex-row gap-2 md:gap-4 justify-center sm:justify-around text-BrandMain-light">
                  <li className="flex flex-col items-center justify-between cursor-pointer">
                    <Link
                      href={
                        "mailto:quantumqeek@gmail.com?subject=Hello%20there&body=I%20wanted%20to%20reach%20out!"
                      }
                      target="_blank"
